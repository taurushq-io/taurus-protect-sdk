# Whitelisted Asset Integrity Verification — TypeScript SDK
...
| Concern | Here |
|---|---|
| Verifier | `src/helpers/whitelisted-asset-verifier.ts` |
| Service | `src/services/whitelisted-asset-service.ts` |
| Asset legacy hashes | `computeAssetLegacyHashes` |
| Parse from verified payload | `parseWhitelistedAssetFromJson` |
| Hash coverage | `verifyHashCoverage` |
| Rule-key resolution | `resolveRuleKey` |
...
**Assets and addresses use DIFFERENT legacy-hash functions.** `computeAssetLegacyHashes` is not the address
...
`resolveRuleKey`. A payload omitting the chain is an error, never a wildcard; a payload disagreeing
...
```
await client.whitelistedAssets.get(assetId)
await client.whitelistedAssets.list({ limit: 50 })
```
