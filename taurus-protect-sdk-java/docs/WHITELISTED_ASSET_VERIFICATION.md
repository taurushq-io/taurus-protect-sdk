# Whitelisted Asset Integrity Verification — Java SDK
...
| Concern | Here |
|---|---|
| Verifier | `client/.../service/WhitelistedAssetService.java` |
| Service | `client/.../service/WhitelistedAssetService.java` |
| Asset legacy hashes | `WhitelistedAssetService.computeLegacyHashes (private)` |
| Parse from verified payload | `AssetHashHelper.parseWhitelistedAssetFromJson` |
| Hash coverage | `SignatureVerifier.verifyHashCoverage` |
| Rule-key resolution | `WhitelistHashHelper.resolveRuleKey` |
| Constant-time compare | BouncyCastle `constantTimeAreEqual` / `MessageDigest.isEqual` |
| Errors | `IntegrityException` / `WhitelistException` |
...
**Assets and addresses use DIFFERENT legacy-hash functions.** `WhitelistedAssetService.computeLegacyHashes (private)` is not the address
...
`WhitelistHashHelper.resolveRuleKey`. A payload omitting the chain is an error, never a wildcard; a payload disagreeing
...
```
client.getWhitelistedAssetService().getWhitelistedAsset(id)
client.getWhitelistedAssetService().getWhitelistedAssets(limit, offset)
```
