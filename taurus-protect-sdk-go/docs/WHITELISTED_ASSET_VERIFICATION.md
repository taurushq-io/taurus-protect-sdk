# Whitelisted Asset Integrity Verification — Go SDK
...
| Concern | Here |
|---|---|
| Verifier | `pkg/protect/helper/whitelisted_asset_verifier.go` |
| Service | `pkg/protect/service/whitelisted_asset.go` |
| Asset legacy hashes | `helper.ComputeAssetLegacyHashes` |
| Parse from verified payload | `helper.ParseWhitelistedAssetFromJSON` |
| Hash coverage | `helper.VerifyHashCoverage` |
| Rule-key resolution | `helper.ResolveRuleKey` |
| Constant-time compare | `helper.ConstantTimeCompare` (crypto/subtle) |
| Errors | `*model.IntegrityError` / `*model.WhitelistError` |
...
**Assets and addresses use DIFFERENT legacy-hash functions.** `helper.ComputeAssetLegacyHashes` is not the address
...
`helper.ResolveRuleKey`. A payload omitting the chain is an error, never a wildcard; a payload disagreeing
...
```
client.WhitelistedAssets().GetWhitelistedAsset(ctx, id)
client.WhitelistedAssets().ListWhitelistedAssets(ctx, opts)
```
