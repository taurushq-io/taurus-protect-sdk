...
| Enforced-in-rules flags | `scripts/resources/enforced-in-rules-vectors.json` (7 service vectors) | `getMe` sends `checkEnforcedInRules=true`; an absent `enforcedInRules` reads false only where the endpoint computes it (getMe, user lists, group reads) and null elsewhere; `publicKeyEnforcedInRules` is never defaulted | all 4 unit suites |
